...
if(!Array.isArray(question_status)) {
  ...
  for (var i = 0; i < json.length; i++) {
    question_status.push({id:i, selected: false ,marked_for_review: false, answer: '', class:"not-visited",code:3});
  }
  if(current_question===null) {
    question_no = 0;
    current_question = {"current_question": question_no}
    localStorage.setItem("question_status", JSON.stringify({"current_question": question_no}));
  }
}
else{
  if(current_question===null) {
    ...
    question_status[0].selected = false;
    question_status[question_no].selected = true;
  }
}
