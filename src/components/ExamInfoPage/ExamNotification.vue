<template>
    <div class="exam-notification card bg-white">
      <h2 class="font-16 text-center primary mb-5">JEE Main Updates and Notifications</h2>
      <div class="divider-horizontal mt-2 mb-2"></div>
      <div class="notifications mb-1">
        <span class="font-15 max-width-90 hover-blue">JEE Main 2020 admit card to be available in the first week of July</span>
        <span class="icon right__icon-black"></span>
      </div>
      <div class="divider-horizontal mt-2 mb-2"></div>
      <div class="notifications mb-1">
        <span class="font-15 max-width-90 hover-blue">Je available in the first week of July</span>
        <span class="icon right__icon-black"></span>
      </div>
      <div class="divider-horizontal mt-2 mb-2"></div>
      <div class="notifications mb-1">
        <span class="font-15 max-width-90 hover-blue">JEE Main 2020 admit card to be available in the first week of July</span>
        <span class="icon right__icon-black"></span>
      </div>
      <div class="divider-horizontal mt-2 mb-2"></div>
      <div class="notifications mb-1">
        <span class="font-15 max-width-90 hover-blue">JEE Main 2020 admit card to be available in the first week of July</span>
        <span class="icon right__icon-black"></span>
      </div> 
    </div>
</template>
